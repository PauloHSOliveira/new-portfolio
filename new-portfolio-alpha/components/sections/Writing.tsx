...
import { Search, ChevronLeft, Clock, Calendar, Hash, ArrowRight, Tag, Bookmark } from 'lucide-react';
        ...
        <article className="space-y-8 max-w-4xl">
            ...
            <div className="flex flex-wrap items-center gap-6 text-[10px] text-[#666] uppercase tracking-[0.2em] font-bold">
              ...
              <div className="flex gap-2">
                {selectedArticle.tags.map(t => (
                  <span key={t} className="text-[#333] border border-[#1a1a1a] px-2 py-0.5 rounded-sm">#{t}</span>
                ))}
              </div>
            </div>
          </header>
          ...
          <footer className="pt-12 mt-12 border-t border-[#1a1a1a] flex flex-col gap-6">
            <div className="bg-[#0f0f0f] border border-[#1a1a1a] p-6 text-[11px] text-[#444] italic leading-relaxed uppercase tracking-wider space-y-2">
              <p>SYSTEM_NOTICE: This document is an MDX-synthesized technical report.</p>
            ...
            >
              <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" /> Close Document & Exit
            </button>
          </footer>
        </article>
      </div>
    ...
    <div className="space-y-12 animate-fadeIn">
      <header className="flex flex-col gap-6">
        <div className="space-y-2">
          <h1 className="text-3xl font-bold uppercase tracking-tight">Technical Writing</h1>
          <p className="text-[#666] text-sm italic tracking-wide">Deep dives into architecture, security primitives, and AI orchestration.</p>
        </div>
      ...
      <div className="grid gap-6">
        {filteredArticles.length > 0 ? (
            ...
            >
              ...
              <div className="flex-1 space-y-4 min-w-0">
                ...
                <div className="flex flex-wrap gap-2 md:pl-7">
                   {art.tags.map(t => (
                     <span key={t} className="text-[8px] text-[#222] group-hover:text-[#444] uppercase font-bold">#{t}</span>
                   ))}
                </div>
              </div>
          ...
          <div className="py-24 text-center border border-dashed border-[#1a1a1a] rounded-sm bg-[#050505]">
            <p className="text-[#444] text-[10px] uppercase tracking-[0.5em]">No documents matching current filters.</p>
          </div>
        )}
      </div>
    </div>
