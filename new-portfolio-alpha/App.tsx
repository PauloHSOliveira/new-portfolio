...
const BootSequence: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
  ...
  return (
    <div className="fixed inset-0 bg-[#050505] z-[100] flex items-center justify-center p-6 md:p-20 font-mono text-[#00ff00]">
      <div className="w-full max-w-2xl h-[400px] overflow-hidden flex flex-col justify-end" aria-live="polite">
        <div className="space-y-1">
          {logs.map((log, i) => (
            <div key={i} className="text-sm md:text-base flex gap-4">
              ...
              <span className={i === BOOT_LOGS.length - 1 ? "font-bold text-white" : ""}>
...
const App: React.FC = () => {
  ...
  const [prompt, setPrompt] = useState('paulo@production:~$ ');
  ...
  useEffect(() => {
    ...
    const sectionDescriptions: Record<Section, string> = {
      about: "Senior Software Engineer and Founder with over 6 years of experience in Fintech, AI, and Distributed Systems.",
      skills: "Analysis of Paulo Oliveira's technological expertise: Node.js, Go, Rust, TypeScript, and AI Orchestration.",
      projects: "Portfolio featuring Paglua (Fintech Infrastructure) and Marcaai (AI Trademark Automation).",
      github: "Global contribution metrics and open-source system repositories on GitHub.",
      writing: "In-depth technical deep-dives on system design, performance, and engineering leadership.",
      contact: "Direct secure channel for architectural consulting and technical leadership opportunities."
    };
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      if (['about', 'skills', 'projects', 'writing', 'github', 'contact'].includes(hash)) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 md:p-10 overflow-hidden relative">
      ...
      {/* Background Decor */}
      <div className="absolute top-10 left-10 text-[10px] text-[#1a1a1a] font-mono pointer-events-none select-none uppercase tracking-[0.5em] hidden xl:block" aria-hidden="true">
        system_architecture_v2.5 // PHOLIVEIRA
      </div>
      <div className="absolute bottom-10 right-10 text-[10px] text-[#1a1a1a] font-mono pointer-events-none select-none uppercase tracking-[0.5em] hidden xl:block" aria-hidden="true">
        node_status: operational // latency: 12ms
      </div>

      <div className="w-full max-w-7xl h-[90vh] bg-[#0a0a0a]/95 backdrop-blur-md border border-[#1a1a1a] rounded-lg shadow-[0_0_50px_rgba(0,0,0,0.8)] flex flex-col animate-fadeIn overflow-hidden relative" role="region" aria-label="Terminal Interface">
        <TerminalHeader />
        
        <div className="flex-1 overflow-y-auto p-6 md:p-12 scroll-smooth custom-scrollbar relative">
          {/* Subtle vignette for focus */}
          <div className="absolute inset-0 pointer-events-none shadow-[inset_0_0_100px_rgba(0,0,0,0.4)]" aria-hidden="true"></div>

          {/* Mock prompt at top */}
          <div className="mb-8 font-mono text-[#00ff00] text-lg font-bold" aria-hidden="true">
            {typedPrompt}
          ...
          <footer className="text-[11px] text-[#444] mt-auto uppercase tracking-wider font-bold border-t border-[#1a1a1a] pt-8 flex justify-between items-center">
            <span>© 2025 Paulo Oliveira — L6 Software Engineer & Architect</span>
            <span className="opacity-50" aria-label="System encoding UTF-8, Connection status Connected">UTF-8 // [CONNECTED]</span>
          </footer>
        </div>
      </div>
